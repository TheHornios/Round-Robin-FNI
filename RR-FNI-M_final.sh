...
fich_random_cfg="datosrangos.txt"
debug2=0
debug3=0
gl_debug=0
asignada=0
...
gl_primera_vez_text_num_part=0
gl_primera_vez_text_num_quantum=0

texto_head_num_part="X-X"
texto_head_tam_part="X-X"
texto_head_tam_quantum="X-X"
texto_head_num_proce="X-X"
texto_head_tiem_llegada="X-X"
texto_head_rafaga="X-X"
texto_head_tam_memoria="X-X"
for arg in "$@"
do
	if [[ "$arg" == "-d" ]]
	then
...
for (( i=0; i < 105; i++ ));do
	...
	if [ $i -eq 14 ];then
		...
		BG_COLORS[14]="\e[48;5;124m"
	elif [ $i -eq 15 ];then
		COLORS[15]="\e[38;5;154m"
		BG_COLORS[15]="\e[48;5;154m"
	elif [[ $i -eq 6 ]]; then
		COLORS[$i]="\e[38;5;99m"
		...
		COLORS[$i]="\e[38;5;59m"
		BG_COLORS[$i]="\e[48;5;69m"
	elif [[ $i -eq 17  ]]; then
		COLORS[$i]="\e[38;5;134m"
		BG_COLORS[$i]="\e[48;5;134m"
	else		
		COLORS[$i]="\e[38;5;${c}m"
...
lectura_fichero_config(){
	...
	crear_fich_temp_num_particiones $arra_cfg
	crear_fich_temp_memrias_random $arra_cfg

	crear_fich_temp_quantum_random $arra_cfg

	crear_fich_temp_num_procesos $arra_cfg
	crear_fich_temp_num_procesos_llegada $arra_cfg
	crear_fich_temp_num_procesos_rafaga $arra_cfg
...
crear_fich_temp_num_procesos(){
	gl_tmp_num_procesos=0

	if [ ${arra_cfg[num_procesos,1]} -eq ${arra_cfg[num_procesos,0]} ]
	then 
		gl_tmp_num_procesos=${arra_cfg[num_procesos,0]}
	else 
		gl_tmp_num_procesos=$(( $RANDOM % ( ${arra_cfg[num_procesos,1]} - ${arra_cfg[num_procesos,0]} ) + ${arra_cfg[num_procesos,0]} ))
	fi

}

crear_fich_temp_num_procesos_llegada(){
	local numero_procesos

	numero_procesos=$gl_tmp_num_procesos
	...
	do
		...
		if [ ${arra_cfg[tiempo_llegada_procesos,1]} -eq ${arra_cfg[tiempo_llegada_procesos,0]} ]
		then 
			tiempo_llegada_procesos=${arra_cfg[tiempo_llegada_procesos,0]}
		
			gl_tmp_num_procesos_llegada[$count]=${tiempo_llegada_procesos}
		else 		
...
crear_fich_temp_num_procesos_rafaga(){
	...
	do
		...
		if [ ${arra_cfg[rafaga_procesos,1]} -eq ${arra_cfg[rafaga_procesos,0]} ]
		then 
			rafaga_procesos=${arra_cfg[rafaga_procesos,0]}
			gl_tmp_num_procesos_rafaga[$count]=${rafaga_procesos}
...
crear_fich_temp_procesos_random(){
	...
	do
		...
		if [ ${arra_cfg[memoria_procesos,1]} -eq ${arra_cfg[memoria_procesos,0]} ]
		then 
			memoria_procesos=${arra_cfg[memoria_procesos,0]}
			gl_tmp_num_procesos_mem[$count]=${memoria_procesos}
		...
		echo "${gl_tmp_num_procesos_llegada[$count]} ${gl_tmp_num_procesos_rafaga[$count]} $memoria_procesos" >> "ultimaRandomRR.temp"
		count=$(( $count + 1 ))
	done
...
crear_fich_temp_quantum_random(){
	...
	gl_tmp_num_quantum=0

	if [ ${arra_cfg[tamanyo_quantum,1]} -eq ${arra_cfg[tamanyo_quantum,0]} ]
	then 
		tamanyo_quantum=${arra_cfg[tamanyo_quantum,0]}
		gl_tmp_num_quantum=${arra_cfg[tamanyo_quantum,0]}
	else 
		tamanyo_quantum=$(( $RANDOM % ( ${arra_cfg[tamanyo_quantum,1]} - ${arra_cfg[tamanyo_quantum,0]} ) + ${arra_cfg[tamanyo_quantum,0]} ))
		gl_tmp_num_quantum="$tamanyo_quantum"
	fi
...
crear_fich_temp_num_particiones(){
	gl_tmp_num_part=0
	if [ ${arra_cfg[num_particiones,1]} -eq ${arra_cfg[num_particiones,0]} ]
	then 
		gl_tmp_num_part=${arra_cfg[num_particiones,0]}
	else 
		gl_tmp_num_part=$(( $RANDOM % ( ${arra_cfg[num_particiones,1]} - ${arra_cfg[num_particiones,0]} ) + ${arra_cfg[num_particiones,0]} ))
	fi
}
...
crear_fich_temp_memrias_random(){
	...
	numero_particiones=$gl_tmp_num_part
	...
	do	
		...
		if [ ${arra_cfg[tamanyo_particiones,1]} -eq ${arra_cfg[tamanyo_particiones,0]} ]
		then 
			tamanyo_particion=${arra_cfg[tamanyo_particiones,0]}
		else 
		...
		gl_tmp_array_part_espacio[$count]=${tamanyo_particion}
...
mostrar_banda_tiempo(){
	...
	#echo $gl_total_espacios
	
	size_gl_total_espacios=$((${#gl_total_espacios}+1))
	size_gl_total_espacios_menos=$((${#gl_total_espacios}))
	...
	for partBT in ${!gl_array_tiempos[*]};do
		...
		for ((i=0 ;  i < $size_gl_total_espacios ; i++ ))
		do
		...
		if [[  $primerSector -ne ${gl_array_tiempos[$partBT]}  &&  ${gl_array_tiempos[$partBT]} -ne -1 ]]; then
			...
			if [ $proceso_BT -lt 10 ];then  
				barraBTextoProcesos[$j_e]="${barraBTextoProcesos[$j_e]}${PROC_COLORS[$proceso_BT]}$(rellenarHuecos "P0${proceso_BT}" $size_gl_total_espacios_menos "n")${color_default}"
				barraBTextoProcesos_nc[$j_e]="${barraBTextoProcesos_nc[$j_e]}$(rellenarHuecos "P0${proceso_BT}" $size_gl_total_espacios_menos "n")"
			else 
				barraBTextoProcesos[$j_e]="${barraBTextoProcesos[$j_e]}${PROC_COLORS[$proceso_BT]}$(rellenarHuecos "P${proceso_BT}" $size_gl_total_espacios_menos "n")${color_default}"
				barraBTextoProcesos_nc[$j_e]="${barraBTextoProcesos_nc[$j_e]}$(rellenarHuecos "P${proceso_BT}" $size_gl_total_espacios_menos "n")"
			fi

			
			barraBTextoTiempos[$j_e]="${barraBTextoTiempos[$j_e]}$(rellenarHuecos "$tiempoActu" $size_gl_total_espacios_menos "s")"
			...
			if [[  $primerSector -ne ${gl_array_tiempos[$partBT]}  &&  ${gl_array_tiempos[$partBT]} -eq -1 ]]; then
				...
				barraBTextoProcesos[$j_e]="${barraBTextoProcesos[$j_e]}$(rellenarHuecos "   " $size_gl_total_espacios_menos "n")" 
				barraBTextoProcesos_nc[$j_e]="${barraBTextoProcesos_nc[$j_e]}$(rellenarHuecos "   " $size_gl_total_espacios_menos "n")"
				primerSector=${gl_array_tiempos[$partBT]}
			else 
				...
				barraBTextoTiempos[$j_e]="${barraBTextoTiempos[$j_e]}$(rellenarHuecos " " $size_gl_total_espacios_menos "s")"
	...
	if [[ $partBtUltimo -eq 0 ]];then 
		...
		barraBTextoTiempos[$j_e]="${barraBTextoTiempos[$j_e]}$(rellenarHuecos "  0" $size_gl_total_espacios_menos "s")"
	else 
		barraBTextoTiempos[$j_e]="${barraBTextoTiempos[$j_e]}$(rellenarHuecos "$partBtUltimo" $size_gl_total_espacios_menos "s")"
	fi 
	

	barraBTextoColores[$j_e]="${barraBTextoColores[$j_e]}$(rellenarHuecos "   " $size_gl_total_espacios_menos "s")"
	barraBTextoColores_nc[$j_e]="${barraBTextoColores_nc[$j_e]}$(rellenarHuecos "   " $size_gl_total_espacios_menos "s")"


	buscar_proceso_ejecucion

	if [[ $procesoEjec -ne -1  ]];then 
		...
		barraBTextoProcesos[$j_e]="${barraBTextoProcesos[$j_e]}$(rellenarHuecos "   " $size_gl_total_espacios_menos "s")"
		barraBTextoProcesos_nc[$j_e]="${barraBTextoProcesos_nc[$j_e]}$(rellenarHuecos "   " $size_gl_total_espacios_menos "s")"
	fi 
...
tabla_ejecucion(){
	...
	for(( xp=0 ; xp < $num_proc ; xp++ ))
	do
		gl_trej_maxima=$(($gl_trej_maxima+${RAFAGA_AUX[$xp]}))
		if [[ ${T_ENTRADA[$xp]} -ge $gl_ttl_maximo ]]; then 
			gl_ttl_maximo=${T_ENTRADA[$xp]}
		fi 

		if [[ ${RAFAGA_AUX[$xp]} -ge $gl_ejecuc_maximo ]]; then 
			gl_ejecuc_maximo=${RAFAGA_AUX[$xp]}
		fi 

		if [[ ${MEMORIA[$xp]} -ge $gl_mens_maxima  ]]; then 
			gl_mens_maxima=${MEMORIA[$xp]}
		fi 

	done

	
	gl_ttl_maximo_len=$((${#gl_ttl_maximo}+1))
	gl_ejecuc_maximo_len=$((${#gl_ejecuc_maximo}+1))
	gl_mens_maxima_len=$((${#gl_mens_maxima}+1))


	gl_tesp_maximo_len=$((${#gl_trej_maxima}+1))

	if [[ $gl_ttl_maximo_len -le 4 ]]; then 
		gl_ttl_maximo_len=4
	fi 

	if [[ $gl_ejecuc_maximo_len -le 4 ]]; then 
		gl_ejecuc_maximo_len=4
	fi 

	if [[ $gl_mens_maxima_len -le 4  ]]; then 
		gl_mens_maxima_len=4
	fi 

	if [[ $gl_tesp_maximo_len -le 5  ]]; then 
		gl_tesp_maximo_len=5
	fi 
	...
	echo -e "\n ╔═════╦$(rellenarHuecos "═════" $gl_ttl_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_ejecuc_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_mens_maxima_len "s" "═")╦$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╦══════╦═══════════════════╗"
	echo -e "\n ╔═════╦$(rellenarHuecos "═════" $gl_ttl_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_ejecuc_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_mens_maxima_len "s" "═")╦$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╦══════╦═══════════════════╗" >> informeRR.txt
	echo -e "\n ╔═════╦$(rellenarHuecos "═════" $gl_ttl_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_ejecuc_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_mens_maxima_len "s" "═")╦$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╦$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╦══════╦═══════════════════╗" >> informeColor.txt
	
	

	echo " ║ Ref ║$(rellenarHuecos "Tll " $gl_ttl_maximo_len "s")║$(rellenarHuecos "Tej " $gl_ejecuc_maximo_len "s")║$(rellenarHuecos "Mem " $gl_mens_maxima_len "s")║$(rellenarHuecos "Tesp " $gl_tesp_maximo_len "s")║$(rellenarHuecos "Tret " $gl_tesp_maximo_len "s")║$(rellenarHuecos "Trej " $gl_tesp_maximo_len "s")║ Part ║       ESTADO      ║"
	echo " ║ Ref ║$(rellenarHuecos "Tll " $gl_ttl_maximo_len "s")║$(rellenarHuecos "Tej " $gl_ejecuc_maximo_len "s")║$(rellenarHuecos "Mem " $gl_mens_maxima_len "s")║$(rellenarHuecos "Tesp " $gl_tesp_maximo_len "s")║$(rellenarHuecos "Tret " $gl_tesp_maximo_len "s")║$(rellenarHuecos "Trej " $gl_tesp_maximo_len "s")║ Part ║       ESTADO      ║" >> informeRR.txt
	echo " ║ Ref ║$(rellenarHuecos "Tll " $gl_ttl_maximo_len "s")║$(rellenarHuecos "Tej " $gl_ejecuc_maximo_len "s")║$(rellenarHuecos "Mem " $gl_mens_maxima_len "s")║$(rellenarHuecos "Tesp " $gl_tesp_maximo_len "s")║$(rellenarHuecos "Tret " $gl_tesp_maximo_len "s")║$(rellenarHuecos "Trej " $gl_tesp_maximo_len "s")║ Part ║       ESTADO      ║" >> informeColor.txt

	
	echo  " ╠═════╬$(rellenarHuecos "═════" $gl_ttl_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_ejecuc_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_mens_maxima_len "s" "═")╬$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╬══════╬═══════════════════╣"
	echo  " ╠═════╬$(rellenarHuecos "═════" $gl_ttl_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_ejecuc_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_mens_maxima_len "s" "═")╬$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╬══════╬═══════════════════╣" >> informeRR.txt
	echo  " ╠═════╬$(rellenarHuecos "═════" $gl_ttl_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_ejecuc_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_mens_maxima_len "s" "═")╬$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╬$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╬══════╬═══════════════════╣" >> informeColor.txt
	...
	do
		...
		gl_total_espacios=$(( $gl_total_espacios + ${T_ENTRADA[$xp]}+${RAFAGA_AUX[$xp]}))
		printf " ║ ${PROC_COLORS[${PROC[$xp]}]}%3s${PROC_COLORS[${PROC[$xp]}]} \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %$(($gl_ttl_maximo_len-1))s${PROC_COLORS[${PROC[$xp]}]} \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %$(($gl_ejecuc_maximo_len-1))s \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %$(($gl_mens_maxima_len-1))s \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]}" $proc_column ${T_ENTRADA[$xp]} ${RAFAGA_AUX[$xp]} ${MEMORIA[$xp]}
		printf " ║ %3s ║ %$(($gl_ttl_maximo_len-1))s ║ %$(($gl_ejecuc_maximo_len-1))s ║ %$(($gl_mens_maxima_len-1))s ║" $proc_column ${T_ENTRADA[$xp]} ${RAFAGA_AUX[$xp]} ${MEMORIA[$xp]} >> informeRR.txt 
		printf " ║ ${PROC_COLORS[${PROC[$xp]}]}%3s${PROC_COLORS[${PROC[$xp]}]} \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %$(($gl_ttl_maximo_len-1))s${PROC_COLORS[${PROC[$xp]}]} \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %$(($gl_ejecuc_maximo_len-1))s \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %$(($gl_mens_maxima_len-1))s \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]}" $proc_column ${T_ENTRADA[$xp]} ${RAFAGA_AUX[$xp]} ${MEMORIA[$xp]} >> informeColor.txt
		...
		printf " %$(($gl_tesp_maximo_len-1))s \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %$(($gl_tesp_maximo_len-1))s \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %$(($gl_tesp_maximo_len-1))s \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %4s \e\033[0;37m║\e[0m" $t_esp $t_ret $raf_print $part
		printf " %$(($gl_tesp_maximo_len-1))s ║ %$(($gl_tesp_maximo_len-1))s ║ %$(($gl_tesp_maximo_len-1))s ║ %4s ║" $t_esp $t_ret $raf_print $part >> informeRR.txt
		printf " %$(($gl_tesp_maximo_len-1))s \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %$(($gl_tesp_maximo_len-1))s \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %$(($gl_tesp_maximo_len-1))s \e\033[0;37m║\e[0m${PROC_COLORS[${PROC[$xp]}]} %4s \e\033[0;37m║\e[0m" $t_esp $t_ret $raf_print $part >> informeColor.txt
		...
		if [[ "${ESTADO[$xp]}" == "Fuera de Sistema" ]];then
			...
			printf "${PROC_COLORS[${PROC[$xp]}]} %-17s \e\033[0;37m║\e[0m" "Fuera del Sistema"
			printf " %-17s${color_defaul} ║\n" "Fuera del Sistema" >> informeRR.txt
			printf "${PROC_COLORS[${PROC[$xp]}]} %-17s \e\033[0;37m║\e[0m\n" "Fuera del Sistema" >> informeColor.txt
		else
			
			printf "${PROC_COLORS[${PROC[$xp]}]} %-17s \e\033[0;37m║\e[0m" "${ESTADO[$xp]}"
	...
	gl_total_espacios=$(( $gl_total_espacios + 1 ))
	
	echo -e "\e\033[0;37m ╚═════╩$(rellenarHuecos "═════" $gl_ttl_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_ejecuc_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_mens_maxima_len "s" "═")╩$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╩══════╩═══════════════════╝\e[0m"
	echo  " ╚═════╩$(rellenarHuecos "═════" $gl_ttl_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_ejecuc_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_mens_maxima_len "s" "═")╩$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╩══════╩═══════════════════╝" >> informeRR.txt
	echo  " ╚═════╩$(rellenarHuecos "═════" $gl_ttl_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_ejecuc_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_mens_maxima_len "s" "═")╩$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╩$(rellenarHuecos "═════" $gl_tesp_maximo_len "s" "═")╩══════╩═══════════════════╝" >> informeColor.txt
		...
		do
			...
			if [[ "${ESTADO[$x]}" == "En Memoria" ]] || [[ "${ESTADO[$x]}" == "En Pausa" ]]; then
				if [[ ${contador_ejecuciones[$x]} -eq $i ]] && [[ ${encolado[$x]} -eq 1 ]]; then
					...
					#gl_cola_de_procesos[$((${#gl_cola_de_procesos[@]}+1))]="$x"
	...
	if [[ $size_gl_total_espacios_menos -le 3 ]]; then 
		size_gl_total_espacios_menos=3
		size_gl_total_espacios=4
	fi 

	size_tam_promedio_part_texto=7
	
	if [[ $size_gl_total_espacios_menos -ge 4 ]]; then 
		count=4

		while [ $size_gl_total_espacios_menos -ge $count ]
		do
			((count++))
			size_tam_promedio_part_texto=$(($size_tam_promedio_part_texto+2))
		done
	...
	for ppart in ${!TAM_PART[*]};do
		...
		if [[ $ind_part_ocupada -ge 0 ]];then
			...
			if [ $procc -lt 10 ];then
				...
				barraProceso[$j]="${barraProceso[$j]}${PROC_COLORS[$procc]}$(rellenarHuecos "P0${procc}" $size_gl_total_espacios_menos "n")${color_default}"
				barraProceso_nc[$j]="${barraProceso_nc[$j]}$(rellenarHuecos "P${procc}" $size_gl_total_espacios_menos "n")"
			else
				#echo -ne "${PROC_COLORS[$procc]}P${procc}${color_default} "
				barraProceso[$j]="${barraProceso[$j]}${PROC_COLORS[$procc]}$(rellenarHuecos "P${procc}" $size_gl_total_espacios_menos "n")${color_default}"
				barraProceso_nc[$j]="${barraProceso_nc[$j]}$(rellenarHuecos "P${procc}" $size_gl_total_espacios_menos "n")"
			fi
			memoria_ocupada=${MEMORIA[$ind_part_ocupada]}
		else
			#echo -n "NA  "
			nada="$(rellenarHuecos "  " $size_gl_total_espacios_menos "n")"
			if [[ $ppart -eq 0 ]]; then
				barraProceso[$j]="${barraProceso[$j]}$(rellenarHuecos "  " $size_gl_total_espacios_menos "n")"
			else
				barraProceso[$j]="${barraProceso[$j]}$(rellenarHuecos "  " $size_gl_total_espacios_menos "n")"
			fi
		...
		if [[ $memoria_ocupada -gt 0 ]];then
			...
			if [[ $parteOcupada -ne ${TAM_PART[$ppart]} ]]; then
				if [[ $disponible -lt 10 ]]; then
					...
					barraTam[$j]="${barraTam[$j]}$(rellenarHuecos "$disponible" $size_gl_total_espacios_menos "s" )"
					barraNumPart[$j]="${barraNumPart[$j]}"
				else
					barraTam[$j]="${barraTam[$j]}$(rellenarHuecos "$disponible" $size_gl_total_espacios_menos "s" )"
					barraNumPart[$j]="${barraNumPart[$j]}"
			...
			while [ $cntparti -lt ${TAM_PART[$ppart]} ];do
				...
				do 
					...
					if [[ $cntparti -lt $((memoria_ocupada)) ]]; then
						...
						if [[ $(( $iteracionRellenar / $size_gl_total_espacios_menos )) -ge 2 ]]; then
							barraTam[$j]="${barraTam[$j]} "
						fi
					fi
					if [[ $cntparti -ge 2 ]]; then
					
						#barraTam[$j]="${barraTam[$j]}c"
						barraNumPart[$j]="${barraNumPart[$j]} "
					fi
		...
		if [[ $sumaParticiones -lt 10 ]]; then
			if [[ $n_par -ne $((ppart + 1)) ]]; then
				barraTam[$j]="${barraTam[$j]}$(rellenarHuecos "$sumaParticiones" $size_gl_total_espacios_menos "s" )"
		...
		if [[ $tamanoPartBarra -lt 10 ]]; then
			if [[ $n_par -ne $((ppart + 1)) ]]; then
				...
				barraNumPart[$j]="${barraNumPart[$j]}$(rellenarHuecos "PART $tamanoPartBarra" $size_tam_promedio_part_texto "n"  )"
				primerSector=1
			fi
		else
			if [[ $n_par -ne $((ppart +1)) ]]; then
	...
	do
		...
		if [[ $i -eq 0 ]]; then
			if [[ $i -eq $j ]]; then
				...
				echo -en "    |$(rellenarHuecos "PART 0" $size_tam_promedio_part_texto "n" )${barraNumPart[$i]}$color_default\n"
				echo -en "    |$(rellenarHuecos "PART 0" $size_tam_promedio_part_texto "n")${barraNumPart[$i]}\n" >> informeRR.txt
				echo -en "    |$(rellenarHuecos "PART 0" $size_tam_promedio_part_texto "n")${barraNumPart[$i]}$color_default\n" >> informeColor.txt
			else
				echo -en "    |$(rellenarHuecos "PART 0" $size_tam_promedio_part_texto "n" )${barraNumPart[$i]}$color_default\n"
				echo -en "    |$(rellenarHuecos "PART 0" $size_tam_promedio_part_texto "n")${barraNumPart[$i]}\n" >> informeRR.txt
		...
		if [[ $i -eq 0 ]]; then
			if [[ $i -eq $j ]]; then
				...
				echo -en "    |$(rellenarHuecos "0" $size_gl_total_espacios_menos "s")${barraTam[$i]}$color_default \n"
				echo -en "    |$(rellenarHuecos "0" $size_gl_total_espacios_menos "s")${barraTam[$i]}\n" >> informeRR.txt
				echo -en "    |$(rellenarHuecos "0" $size_gl_total_espacios_menos "s")${barraTam[$i]}$color_default\n" >> informeColor.txt
			else
				echo -en "    |$(rellenarHuecos "0" $size_gl_total_espacios_menos "s")${barraTam[$i]}$color_default\n"
				echo -en "    |$(rellenarHuecos "0" $size_gl_total_espacios_menos "s")${barraTam[$i]}\n" >> informeRR.txt
...
pedir_procesos_rangos() {
	...
	textos_pedidato[1]=" Introduzca el rango de tiempo de llegada de los procesos  (Ej: 4-10): "
	textos_pedidato[2]=" Introduzca el rengo de tiempo de ejecucion de los procesos(Ej: 2-16): "
	textos_pedidato[3]=" Introduzca el rango de memoria de los procesos  (Ej: 2-7): "
	...
	do
		...
		clear
		imprime_cabecera
		imprimir_cabecera_rangos
		while  [ $bucle_pedir_num == -1 ] ;
		do
			
			read -p "${textos_pedidato[$count_p]}" leer_dato_usuario
			echo "		  ${textos_pedidato[$count_p]}" >> informeRR.txt
			echo "		  ${textos_pedidato[$count_p]}" >> informeColor.txt
			...
			then
				...
				min_num[$count_p,0]=${arrIN[0]} 
				max_num[$count_p,1]=${arrIN[1]} 


				if [[  $count_p -eq 3  && tamanyoMaximoParticion -lt ${max_num[$count_p,1]} ]]; then 
					echo -e "\e[31m El tamaño de los procesos no puede ser mayor que el de la partición de mayor tamaño  \e[0m \n"
					echo "		  El tamaño de los procesos no puede ser mayor que el de la partición de mayor tamaño  " >> informeRR.txt
					echo "		  El tamaño de los procesos no puede ser mayor que el de la partición de mayor tamaño  " >> informeColor.txt
					...
					if	 [ ${min_num[$count_p,0]} == -1 ] || [  ${min_num[$count_p,0]} -gt  ${max_num[$count_p,1]} ]
					then
						...
						# $gl_tmp_num_procesos
						# $gl_tmp_num_procesos_llegada
						# $gl_tmp_num_procesos_rafaga
						# $gl_tmp_num_procesos_rafaga

						if [[ $count_p -eq 0 ]]; then 
							arra_cfg[num_procesos,0]=${min_num[$count_p,0]}
							arra_cfg[num_procesos,1]=${max_num[$count_p,1]}
							texto_head_num_proce="$leer_dato_usuario"
							crear_fich_temp_num_procesos $arra_cfg
						fi 
						if [[ $count_p -eq 1 ]]; then 

							arra_cfg[tiempo_llegada_procesos,0]=${min_num[$count_p,0]}
							arra_cfg[tiempo_llegada_procesos,1]=${max_num[$count_p,1]}
							texto_head_tiem_llegada="$leer_dato_usuario"
							crear_fich_temp_num_procesos_llegada $arra_cfg
						fi 
						if [[ $count_p -eq 2 ]]; then 
							arra_cfg[rafaga_procesos,0]=${min_num[$count_p,0]}
							arra_cfg[rafaga_procesos,1]=${max_num[$count_p,1]}
							texto_head_rafaga="$leer_dato_usuario"
							crear_fich_temp_num_procesos_rafaga $arra_cfg
						fi 
						if [[ $count_p -eq 3 ]]; then 
							arra_cfg[memoria_procesos,0]=${min_num[$count_p,0]}
							arra_cfg[memoria_procesos,1]=${max_num[$count_p,1]}
							texto_head_tam_memoria="$leer_dato_usuario"
							crear_fich_temp_procesos_random $arra_cfg
						fi 
						bucle_pedir_num=1
					fi
...
pedir_memoria_rangos() {
	...
	do
		...
		read -p " Introduzca el rango del numero de particiones  (Ej: 1-20): " leer_dato_usuario
	
		echo "		  Introduzca el rango del numero de particiones  (Ej: 1-20): " >> informeRR.txt
		echo "		  Introduzca el rango del numero de particiones  (Ej: 1-20): " >> informeColor.txt
		...
		then
			...
			if	 [ $num_min_particiones == -1 ] || [  $num_min_particiones -gt  $num_max_particiones ]
			then
				echo -e "\e[31m El primer número tiene que ser mayor de 1 y el segundo no puede ser menor del primero  \e[0m \n"
				echo "		  El primer número tiene que ser mayor de 1 y el segundo no puede ser menor del primero" >> informeRR.txt
				echo "		  El primer número tiene que ser mayor de 1 y el segundo no puede ser menor del primero" >> informeColor.txt
				...
				texto_head_num_part="$leer_dato_usuario"
				bucle_pedir_num_particiones=1
			fi
	...
	arra_cfg[num_particiones,0]=${num_min_particiones}
	arra_cfg[num_particiones,1]=${num_max_particiones}
	...
	do
		...
		then
			...
			if	 [ $tam_min_particiones == -1 ] || [  $tam_min_particiones -gt  $tam_max_particiones ]
			then
				echo -e "\e[31m El primer número tiene que ser mayor de 1 y el segundo no puede ser menor del primero  \e[0m \n"
				echo "		  El primer número tiene que ser mayor de 1 y el segundo no puede ser menor del primero" >> informeRR.txt
				echo "		  El primer número tiene que ser mayor de 1 y el segundo no puede ser menor del primero" >> informeColor.txt
				...
				texto_head_tam_part="$leer_dato_usuario"
				bucle_pedir_tam_particiones=1
			fi
	...
	arra_cfg[tamanyo_particiones,0]=${tam_min_particiones}
	arra_cfg[tamanyo_particiones,1]=${tam_max_particiones}
...
pedir_quantum_rangos() {
	...
	do
		...
		then
			...
			if	 [ $min_quantum == -1 ] || [  $min_quantum -gt  $max_quantum ]
			then
				echo -e "\e[31m El primer número tiene que ser mayor de 1 y el segundo no puede ser menor del primero  \e[0m \n"
				echo "		  El primer número tiene que ser mayor de 1 y el segundo no puede ser menor del primero" >> informeRR.txt
				echo "		  El primer número tiene que ser mayor de 1 y el segundo no puede ser menor del primero" >> informeColor.txt
				...
				texto_head_tam_quantum="$leer_dato_usuario"
				bucle_pedir_quantum=1
			fi
	...
	arra_cfg[tamanyo_quantum,0]=${min_quantum}
	arra_cfg[tamanyo_quantum,1]=${max_quantum}
...
mostrar_menu_opciones_algoritmo(){
	...
	echo " [1] Por Eventos (Pulsando enter)"	
	echo " [2] Automatico (Esperando un tiempo X entre volcados)"
	echo " [3] Completo (Ver los volcados sin espera alguna)"
}

mostrar_menu_opciones(){
	echo " Selecciona una de las dos opciones (a,b):"
	echo " [1] Introducir los datos por teclado"	
	echo " [2] Introducir los datos desde el fichero de datos de ultima ejecucion (datos.txt)"
	echo " [3] Introducir los datos desde otros ficheros de datos"
	echo " [4] Generar datos aleatorios a partir de rangos introducidos de forma manual"
	echo " [5] Introducir los datos desde el fichero de rangos de ultima ejecucion (datosrangos.txt)"
	echo " [6] Introducir los rangos desde otro fichero de rangos"
	...
	echo "		  [2] Introducir los datos desde el fichero de datos de ultima ejecucion (datos.txt)" >> informeRR.txt
	echo "		  [3] Introducir los datos desde otros ficheros de datos" >> informeRR.txt
	echo "		  [4] Generar datos aleatorios a partir de rangos introducidos de forma manual" >> informeRR.txt
	echo "		  [5] Introducir los datos desde el fichero de rangos de ultima ejecucion (datosrangos.txt)" >> informeRR.txt
	echo "		  [6] Introducir los rangos desde otro fichero de rangos" >> informeRR.txt
	...
	echo "		  [2] Introducir los datos desde el fichero de datos de ultima ejecucion (datos.txt)" >> informeColor.txt
	echo "		  [3] Introducir los datos desde otros ficheros de datos" >> informeColor.txt
	echo "		  [4] Generar datos aleatorios a partir de rangos introducidos de forma manual" >> informeColor.txt
	echo "		  [5] Introducir los datos desde el fichero de rangos de ultima ejecucion (datosrangos.txt)" >> informeColor.txt
	echo "		  [6] Introducir los rangos desde otro fichero de rangos" >> informeColor.txt
...
preguntar_por_archivo_a_guardar_cfg(){
	archivo_default_cfg="datosrangos.txt"
	clear
	imprime_cabecera

	opt_preguntar_por_archivo_a_guardar=0
	echo " Selecciona en que fichero quieres guardar los rangos de la última ejecución" 
	echo " [1] Fichero de rangos por defecto (datosrangos.txt)" 
	echo " [2] Introduce el nombre del fichero de rangos por teclado" 

	echo " Selecciona en que fichero quieres guardar los rangos de la última ejecución"   >> informeRR.txt
	echo " [1] Fichero de rangos por defecto (datosrangos.txt)"   >> informeRR.txt
	echo " [2] Introduce el nombre del fichero de rangos por teclado"   >> informeRR.txt

	echo " Selecciona en que fichero quieres guardar los rangos de la última ejecución"   >> informeColor.txt
	echo " [1] Fichero de rangos por defecto (datosrangos.txt)"   >> informeColor.txt
	echo " [2] Introduce el nombre del fichero de rangos por teclado"   >> informeColor.txt

	read -p " Selecciona una de las opciones:" opt_preguntar_por_archivo_a_guardar
	echo " Teclado: $opt_preguntar_por_archivo_a_guardar" >> informeRR.txt
	echo " Teclado: $opt_preguntar_por_archivo_a_guardar" >> informeColor.txt


	while (( opt_preguntar_por_archivo_a_guardar < 1 || opt_preguntar_por_archivo_a_guardar > 2 ));
	do
		echo " Entrada no válida, sólo 1 y 2 admitido"
		echo " Selecciona en que fichero quieres guardar los rangos de la última ejecución" 
		echo " [1] Fichero de rangos por defecto (datosrangos.txt)" 
		echo " [2] IIntroduce el nombre del fichero de rangos por teclado" 


		echo " Entrada no válida, sólo 1 y 2 admitido" >> informeRR.txt
		echo " Selecciona en que fichero quieres guardar los rangos de la última ejecución"  >> informeRR.txt
		echo " [1] Fichero de rangos por defecto (datdatosrangosos.txt)"  >> informeRR.txt
		echo " [2] Introduce el nombre del fichero de rangos por teclado"  >> informeRR.txt

		echo " Entrada no válida, sólo 1 y 2 admitido" >> informeColor.txt
		echo " Selecciona en que fichero quieres guardar los rangos de la última ejecución"  >> informeColor.txt
		echo " [1] Fichero de rangos por defecto (datosrangos.txt)"  >> informeColor.txt
		echo " [2] Introduce el nombre del fichero de rangos por teclado"  >> informeColor.txt
	...
	if [[ $opt_preguntar_por_archivo_a_guardar -eq 2 ]]; then 
		read -p " Introduce el nombre del archivo y su extension que va a almacenar los datos:" archivo_default_cfg
	fi 

}

preguntar_por_archivo_a_guardar(){
	archivo_default='datos.txt'
	clear
	imprime_cabecera

	opt_preguntar_por_archivo_a_guardar=0
	echo " Selecciona en que archivo quieres guardar los datos de la ejecucción" 
	echo " [1] Archivo por defecto (datos.txt)" 
	echo " [2] Introduce el nombre del archivo por teclado" 

	echo " Selecciona en que archivo quieres guardar los datos de la ejecucción"   >> informeRR.txt
	echo " [1] Archivo por defecto (datos.txt)"   >> informeRR.txt
	echo " [2] Introduce el nombre del archivo por teclado"   >> informeRR.txt

	echo " Selecciona en que archivo quieres guardar los datos de la ejecucción"   >> informeColor.txt
	echo " [1] Archivo por defecto (datos.txt)"   >> informeColor.txt
	echo " [2] Introduce el nombre del archivo por teclado"   >> informeColor.txt
	...
	do
		...
		echo " Selecciona en que archivo quieres guardar los datos de la ejecucción" 
		echo " [1] Archivo por defecto (datos.txt)" 
		echo " [2] IIntroduce el nombre del archivo por teclado" 
	...
	if [[ $opt_preguntar_por_archivo_a_guardar -eq 2 ]]; then 
		read -p " Introduce el nombre del archivo y su extension que va a almacenar los datos:" archivo_default
	fi 
	...
	then #RR
		...
		then # En el caso de que el fichero exista tambien hay que compobar que esta vacio
			...
			then # El archivo si que tiene contenido
				...
				echo -e "\e[31m  El archivo ('datos.txt') no tiene contenido  \e[0m \n"
				lee_datos_menu
			...
			echo -e "\e[31m  No existe fichero de ultima ejecución ('datos.txt') \e[0m \n"
			lee_datos_menu
		...
		then # En el caso de que el fichero exista tambien hay que compobar que esta vacio
			...
			then # El archivo si que tiene contenido
				...
				echo -e "\e[31m  El archivo ('datos.txtv') no tiene contenido  \e[0m \n"
				lee_datos_menu
			...
			echo -e "\e[31m  No existe fichero de ultima ejecución ('datos.txtv') \e[0m \n"
			lee_datos_menu
...
# Opcion 4: es la opcion de crear el archivo cfg que va a tener los rangos para generar aleatoriamente
menu_opcion_4() {
	...
	preguntar_por_archivo_a_guardar_cfg
	preguntar_por_archivo_a_guardar
	fich_random_cfg=$archivo_default_cfg
	...
	if [ -f "ultimaRandomRR.temp" ];
	then
		rm "ultimaRandomRR.temp"
	fi

	if [ -f "$fich_random_cfg" ];
	then
		rm "$fich_random_cfg"
	fi
	...
	rm "ultimaRandomRR.temp"
	#lectura_fichero_config "datosrangos.txt"

}

# Funcion que muestra la cabecera (menu) de la opcion de random
imprimir_cabecera_rangos() {

	echo " ╔════════════════════════════════════════════╗"
	echo " ║ Valores de rangos introducidos             ║"
	echo " ╠═══════════════════════════╦════════════════║"


	echo " ╔════════════════════════════════════════════╗"  >> informeRR.txt
	echo " ║ Valores de rangos introducidos             ║"  >> informeRR.txt
	echo " ╠═══════════════════════════╦════════════════║"  >> informeRR.txt


	echo " ╔════════════════════════════════════════════╗"  >> informeColor.txt
	echo " ║ Valores de rangos introducidos             ║"  >> informeColor.txt
	echo " ╠═══════════════════════════╦════════════════║"  >> informeColor.txt
			
	imprimir_cabecera_rangos_numero_usuario
...
imprimir_cabecera_rangos_numero_usuario(){
	...
	if [[ "$gl_tmp_num_part" =~ ^[0-9]+$ && $gl_primera_vez_text_num_part -eq 0 ]];then 
		texto_head_num_part=$(rellenarHuecos "$texto_head_num_part = $(( $gl_tmp_num_part + 1 ))" 14 "n")
		gl_primera_vez_text_num_part=1
	else 
		texto_head_num_part=$(rellenarHuecos "$texto_head_num_part" 14 "n")
	fi

	texto_head_tam_part=$(rellenarHuecos "$texto_head_tam_part" 14 "n")

	if [[ "$gl_tmp_num_quantum"  =~ ^[0-9]+$ && $gl_primera_vez_text_num_quantum -eq 0 ]]; then
		texto_head_tam_quantum=$(rellenarHuecos "$texto_head_tam_quantum = $gl_tmp_num_quantum" 14 "n")
		gl_primera_vez_text_num_quantum=1
	else 
		texto_head_tam_quantum=$(rellenarHuecos "$texto_head_tam_quantum" 14 "n")
	fi 

	texto_head_num_proce=$(rellenarHuecos "$texto_head_num_proce" 14 "n")
	texto_head_tiem_llegada=$(rellenarHuecos "$texto_head_tiem_llegada" 14 "n")
	texto_head_rafaga=$(rellenarHuecos "$texto_head_rafaga" 14 "n")
	texto_head_tam_memoria=$(rellenarHuecos "$texto_head_tam_memoria" 14 "n")


	texto_head_paticiones_generadas=""

	if [[ "$gl_tmp_num_quantum"  =~ ^[0-9]+$ ]]; then
		texto_head_paticiones_generadas=" QUANTUM = $gl_tmp_num_quantum   $texto_head_paticiones_generadas"
	fi 

	if [[ "$gl_tmp_num_part" =~ ^[0-9]+$ ]];then 
		count=0
		while [ $count -le $gl_tmp_num_part ]
		do	

			if [[ "$gl_tmp_array_part_espacio" ]];then 
				texto_head_paticiones_generadas="$texto_head_paticiones_generadas PART $count = ${gl_tmp_array_part_espacio[$count]}"
			else 
				texto_head_paticiones_generadas="$texto_head_paticiones_generadas PART $count = X"
				#texto_head_paticiones_generadas="$texto_head_paticiones_generadas PART $count = ${gl_tmp_array_part_espacio[$count]}"
			fi 
			count=$(( $count + 1 ))
		done
	...
	echo " ║ Numero de particiones     ║ $texto_head_num_part║"
	echo " ╠═══════════════════════════╬════════════════║"
	echo " ║ Tamaño de las particiones ║ $texto_head_tam_part║"
	echo " ╠═══════════════════════════╬════════════════║"
	echo " ║ Quantum                   ║ $texto_head_tam_quantum║"
	echo " ╠═══════════════════════════╬════════════════║"
	echo " ║ Numero de procesos        ║ $texto_head_num_proce║"
	echo " ╠═══════════════════════════╬════════════════║"
	echo " ║ Tiempo de llegada         ║ $texto_head_tiem_llegada║"
	echo " ╠═══════════════════════════╬════════════════║"
	echo " ║ Tiempo de ejecución       ║ $texto_head_rafaga║"
	echo " ╠═══════════════════════════╬════════════════║"
	echo " ║ Memoria de los procesos   ║ $texto_head_tam_memoria║"
	echo " ╚═══════════════════════════╩════════════════╝"

	echo " ║ Numero de particiones     ║ $texto_head_num_part║" >> informeRR.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeRR.txt
	echo " ║ Tamaño de las particiones ║ $texto_head_tam_part║" >> informeRR.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeRR.txt
	echo " ║ Quantum                   ║ $texto_head_tam_quantum║" >> informeRR.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeRR.txt
	echo " ║ Numero de procesos        ║ $texto_head_num_proce║" >> informeRR.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeRR.txt
	echo " ║ Tiempo de llegada         ║ $texto_head_tiem_llegada║" >> informeRR.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeRR.txt
	echo " ║ Tiempo de ejecución       ║ $texto_head_rafaga║" >> informeRR.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeRR.txt
	echo " ║ Memoria de los procesos   ║ $texto_head_tam_memoria║" >> informeRR.txt
	echo " ╚═══════════════════════════╩════════════════╝" >> informeRR.txt

	echo " ║ Numero de particiones     ║ $texto_head_num_part║" >> informeColor.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeColor.txt
	echo " ║ Tamaño de las particiones ║ $texto_head_tam_part║" >> informeColor.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeColor.txt
	echo " ║ Quantum                   ║ $texto_head_tam_quantum║" >> informeColor.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeColor.txt
	echo " ║ Numero de procesos        ║ $texto_head_num_proce║" >> informeColor.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeColor.txt
	echo " ║ Tiempo de llegada         ║ $texto_head_tiem_llegada║" >> informeColor.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeColor.txt
	echo " ║ Tiempo de ejecución       ║ $texto_head_rafaga║" >> informeColor.txt
	echo " ╠═══════════════════════════╬════════════════║" >> informeColor.txt
	echo " ║ Memoria de los procesos   ║ $texto_head_tam_memoria║" >> informeColor.txt
	echo " ╚═══════════════════════════╩════════════════╝" >> informeColor.txt


	if [[ "$gl_tmp_num_part" =~ ^[0-9]+$ ]];then 
		echo " ╔═════════════════════════════════════╗"
		echo " ║ Particiones y quantum generado      ║"
		echo " ╚═════════════════════════════════════╝"
		echo "$texto_head_paticiones_generadas "
		echo ""

		echo " ╔═════════════════════════════════════╗"  >> informeRR.txt
		echo " ║ Particiones y quantum generado      ║"  >> informeRR.txt
		echo " ╚═════════════════════════════════════╝"  >> informeRR.txt
		echo "$texto_head_paticiones_generadas "  >> informeRR.txt
		echo ""  >> informeRR.txt

		echo " ╔═════════════════════════════════════╗" >> informeColor.txt
		echo " ║ Particiones y quantum generado      ║" >> informeColor.txt
		echo " ╚═════════════════════════════════════╝" >> informeColor.txt
		echo "$texto_head_paticiones_generadas " >> informeColor.txt
		echo "" >> informeColor.txt
	fi


	if [[ "$gl_tmp_num_procesos" =~ ^[0-9]+$ ]];then 
		echo " ╔═══════════════════════╗"
		echo " ║ Procesos generados    ║"
		echo " ╠═════╦═════╦═════╦═════╣"
		echo " ║ Ref ║ Tll ║ Tej ║ Mem ║"
		echo " ╠═════╬═════╬═════╬═════╣"

		echo " ╔═══════════════════════╗"  >> informeRR.txt
		echo " ║ Procesos generados    ║"  >> informeRR.txt
		echo " ╠═════╦═════╦═════╦═════╣"  >> informeRR.txt
		echo " ║ Ref ║ Tll ║ Tej ║ Mem ║"  >> informeRR.txt
		echo " ╠═════╬═════╬═════╬═════╣"  >> informeRR.txt

		echo " ╔═══════════════════════╗" >> informeColor.txt
		echo " ║ Procesos generados    ║" >> informeColor.txt
		echo " ╠═════╦═════╦═════╦═════╣" >> informeColor.txt
		echo " ║ Ref ║ Tll ║ Tej ║ Mem ║" >> informeColor.txt
		echo " ╠═════╬═════╬═════╬═════╣" >> informeColor.txt

		texto_head_procesos_henerados=""
		count=0
		while [ $count -le $gl_tmp_num_procesos ]
		do	
			gl_tmp_num_procesos_text=$(( $count+1 ))
			if [[ $gl_tmp_num_procesos_text -lt 10 ]]; then
				gl_tmp_num_procesos_text="0$gl_tmp_num_procesos_text"
			fi 

			texto_head_procesos_henerados="$texto_head_procesos_henerados ║ P$gl_tmp_num_procesos_text ║ $(rellenarHuecos "${gl_tmp_num_procesos_llegada[$count]}" 2 "s") ║ $(rellenarHuecos "${gl_tmp_num_procesos_rafaga[$count]}" 2 "s") ║ $(rellenarHuecos "${gl_tmp_num_procesos_mem[$count]}" 2 "s") ║\n"
			
			count=$(( $count + 1 ))
		done

		echo -e "$texto_head_procesos_henerados ╚═════╩═════╩═════╩═════╝"
		echo -e "$texto_head_procesos_henerados ╚═════╩═════╩═════╩═════╝"  >> informeRR.txt
		echo -e "$texto_head_procesos_henerados ╚═════╩═════╩═════╩═════╝" >> informeColor.txt
		# $gl_tmp_num_procesos
		# $gl_tmp_num_procesos_llegada
		# $gl_tmp_num_procesos_rafaga
		# $gl_tmp_num_procesos_rafaga
...
menu_opcion_5() {
	...
	if [ -f 'datosrangos.txt' ]
	then # En el caso de que el fichero exista tambien hay que compobar que esta vacio

		if [ -s 'datosrangos.txt' ]
		then # El archivo si que tiene contenido

				
			lectura_fichero_config "datosrangos.txt" # Leemos los datos del fichero

		else # El alchivo esta vacio

			echo -e "\e[31m  El archivo ('datosrangos.txt') no tiene contenido  \e[0m \n"
			#lee_datos_menu
		...
		echo -e "\e[31m  No existe fichero de ultima ejecución ('datosrangos.txt') \e[0m \n"
		#lee_datos_menu
...
hay_procesos_en_cola(){

	local pr
	gl_hay_cola=-1 # Asuncion de que no hay ninguno
	for((pr=0 ; pr < $num_proc ; pr++ ))
	do
		if [[ "${ESTADO[$pr]}" == "En Memoria" ]] || [[ "${ESTADO[$pr]}" == "En Pausa" ]]; then
			...
			do
				...
				if [[ ${ESTADO[$proceso_actual]} == Finalizado ]];then # l proceso ya está acabado, lo ignoramos
					...
					if [ $asignada -eq 0 ];then # Solo asigna en el caso que este libre la particion al proceso
						...
						if [[ $gl_primero_en_ejecucion -ne -1 && $gl_quantum_restante -eq 1 && $gl_hay_cola -eq -1 ]]
						then 
							gl_quantum_restante=`expr $quantum + 1`
						fi 
			...
			do
				...
				if [[ ${ESTADO[$proceso_actual]} == Acabado ]];then
					...
					if [ $asignada -eq 0 ];then # Solo asigna en el caso que este libre la particion al proceso
						...
						#if [[ $gl_iteracion -eq 5 ]]
						#then 
						#	tabla_ejecucion
						#	echo "--problema encolar"
						#	echo "--$gl_quantum_restante"
							#exit
						#fi 
...
if [[ $dat_fich -eq 4 ]];then 
	#
	archivo_default=$archivo_default
else 
	archivo_default='datos.txt'
fi 
...
then
	...
	if [ $opcion = "a" ];then #RR
		rm $archivo_default 
		cp ultimosRR.txt $archivo_default #datos.txt
		rm ultimosRR.txt
	else #RRV
		rm ultimos RR.rrv
		mv ultimosRR.txt $archivo_default #datos.txtv
	fi
fi
datos_aux
